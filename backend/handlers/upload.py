...
async def handle_upload(file: UploadFile = File(...)):
    if file.content_type != "application/pdf":
        logger.error(f"{file.filename or "uploaded_pdf"} Invalid file type. Please upload a PDF file.")
        raise HTTPException(
                status_code=400,
    ...
    try:
        ...
        if not extracted_text:
            logger.warning(f"{file.filename or "uploaded_pdf"} No text could be extracted.")
            return {
                    "filename": file.filename or "uploaded_pdf",
                    "extracted_text": "No text could be extracted. The PDF might be image-based, password-protected, or corrupted."
                    }
